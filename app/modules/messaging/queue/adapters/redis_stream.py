...
# Sentinel field stored alongside the user payload so we can carry the
# receive-count across requeue (XACK + XADD creates a new stream entry id).
# Stripped from ``QueueMessage.body`` before the message is handed back.
_RECEIVE_COUNT_FIELD = "_receive_count"
...
class RedisStreamQueueGateway:
        ...
        *,
        ...
        for _stream_name, entries in response:
            for raw_id, fields in entries:
                ...
                receive_count = int(body.pop(_RECEIVE_COUNT_FIELD, 1))
                messages.append(
                    QueueMessage(
                        id=message_id,
                        body=body,
    ...
    async def nack(self, message: QueueMessage, *, requeue: bool = True) -> None:
        ...
        if requeue:
            ...
            body[_RECEIVE_COUNT_FIELD] = message.receive_count + 1
