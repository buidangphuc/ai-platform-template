...
def _build_app(handler) -> FastAPI:
    return create_app(handler, settings=_settings(), init_resources=True)
...
async def test_async_completion_returns_503_when_tasks_disabled():
    handler = _RecordingHandler()
    settings = _settings().model_copy(update={"TASKS_ENABLED": False})
    app = create_app(handler, settings=settings, init_resources=True)
